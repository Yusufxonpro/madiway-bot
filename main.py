...
YUK_SETTINGS_FILE = "yuk_settings_images.json" # Ko'p rasmli yangi fayl
...
def get_next_photo_id():
    ...
    # Indeksni keyingi safar uchun yangilaymiz
    cfg["current_index"] = (idx + 1) % len(images)
    save_yuk_settings(cfg)
    return photo_id
...
async def btns(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "btn_sotib_olish":
        ...
        cfg = load_yuk_settings()
        current_count = len(cfg.get("images", []))
        await callback.message.answer(
            f"📸 <b>Ayni paytda bazada {current_count} ta rasm bor.</b>\n"
            f"Yangi rasmni yuboring (Rasm avtomatik ro'yxatga qo'shiladi. 10 tadan oshsa, eskilari yangilanadi):"
        )
        await state.set_state(MadiWayStates.kutish_yuk_photo)
    elif callback.data == "btn_kanal_tashlash":
        await callback.message.answer("📥 Kanal yukini yuboring (Matn):")
        await state.set_state(MadiWayStates.kutish_kanal_yuk)
    elif callback.data == "btn_hamma_topic":
        await callback.message.answer("💥 Hamma bo'lim va oylarga yuboriladigan yukni yuboring:")
        await state.set_state(MadiWayStates.kutish_hamma_topic_yuk)
    elif callback.data == "btn_kanal_va_hamma":
        await callback.message.answer("🚀 Kanalga va Hamma joyga ketadigan yukni yuboring:")
        await state.set_state(MadiWayStates.kutish_kanal_va_hamma_topic)
    elif callback.data == "btn_bitta_topic":
        # Hududlar va oylarni chiroyli qator qilib chiqarish
        ...
        await callback.message.answer("📍 Bo'lim yoki Oyni tanlang:", reply_markup=kb)
...
# 10 tagacha rasm yig'ish va saqlash logikasi
...
async def save_yuk_photo(message: types.Message, state: FSMContext):
    if message.photo:
        ...
        if new_photo_id not in images:
            if len(images) >= 10:
                images.pop(0) # 10 tadan oshganda eng eskisini o'chiradi
            images.append(new_photo_id)
            cfg["images"] = images
            save_yuk_settings(cfg)
            await message.answer(f"✅ Rasm bazaga qo'shildi! Jami rasmlar: {len(images)}/10 ta.")
        else:
            await message.answer("ℹ️ Bu rasm allaqachon bazada mavjud.")
    else:
        await message.answer("❌ Iltimos, faqat rasm (photo) yuboring.")
    await state.clear()

# --- KAFILLI YUBORISH TIZIMI ---
async def send_all(chat_id, caption, kb, t_id=None):
    # Har xabar ketganda navbatdagi rasmni olamiz (Almashtirib tashlash mexanizmi)
    ...
    try:
        if rotated_photo_id:
            await bot.send_photo(chat_id=chat_id, photo=rotated_photo_id, caption=caption, reply_markup=kb, message_thread_id=thread_id)
        else:
        ...
        if "thread" in str(e).lower() and thread_id is not None:
            try:
                if rotated_photo_id: await bot.send_photo(chat_id=chat_id, photo=rotated_photo_id, caption=caption, reply_markup=kb)
                else: await bot.send_message(chat_id=chat_id, text=caption, reply_markup=kb)
                return True
            except Exception: return False
        return False

# --- MUDDAT VA YUBORISH JARAYONI ---
...
async def process_yuk_content(message: types.Message, state: FSMContext):
    ...
    await state.update_data(yuk_text=txt, prev_state=current_state)
...
async def process_duration(message: types.Message, state: FSMContext):
    ...
    if prev_state == MadiWayStates.kutish_kanal_yuk.state:
        ...
        res = await send_all(CHANNEL_ID, cap, get_channel_kb(m_id))
        if res: success_flag = True

    # 2. BITTA TOPIC/OYGA TASHLASH
        ...
        res = await send_all(GROUP_ID, cap, get_channel_kb(), tid)
        if res: success_flag = True

    # 3. HAMMA INTERFEYSGA (TOPIC + OYLAR) TASHLASH
        ...
        for n, tid in TOPICS.items():
            res = await send_all(GROUP_ID, cap, get_channel_kb(), tid)
            if res: sent_count += 1
            await asyncio.sleep(0.3) # Telegram limitidan himoya
        if sent_count > 0: success_flag = True

    # 4. KANAL + HAMMA JOYGA TASHLASH
        ...
        await send_all(CHANNEL_ID, cap_chan, get_channel_kb(m_id))
        
        cap_group = get_premium_caption(txt, duration_text=duration)
        sent_count = 0
        for n, tid in TOPICS.items():
            res = await send_all(GROUP_ID, cap_group, get_channel_kb(), tid)
            if res: sent_count += 1
            await asyncio.sleep(0.3)
        if sent_count > 0: success_flag = True

    if success_flag:
        await message.answer("🚀 <b>Muvaffaqiyatli tasdiqlandi! Rasmlar navbat bilan almashtirilib yuborildi.</b>", reply_markup=types.ReplyKeyboardRemove())
    else:
...
async def main():
    print("-----------------------------------------")
    print("MADIWAY Karusel-Rasm va Oylar tizimi faol.")
    print("-----------------------------------------")
